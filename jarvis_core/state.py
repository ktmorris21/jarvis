...
class JarvisState:
    def __init__(self):
        self.started_at=utc_now(); self.interfaces={}; self.user_present=False; self.last_user_activity=None; self.last_interaction=None; self.social_drive=.10; self.boredom=0; self.event_count=0; self.recent_events=[]; self.last_cognition_action=None; self.last_cognition_reason=None; self.last_cognition_at=None; self.last_command_result=None; self.last_formed_memory_ids=[]; self.last_retrieved_memory_ids=[]; self.last_attention=None; self.last_context=None; self._lock=asyncio.Lock()
    def restore(self):
        u=repository.get_state("person:user") or {}; j=repository.get_state("jarvis:core") or {}; self.user_present=bool(u.get("present",False)); self.social_drive=float(j.get("social_drive",.1)); self.boredom=float(j.get("boredom",0)); self.recent_events=[x["type"] for x in repository.recent_events(20)]
    def persist_runtime(self):
        ...
        repository.upsert_state("jarvis:core","jarvis",{"social_drive":round(self.social_drive,4),"boredom":round(self.boredom,4),"last_cognition_action":self.last_cognition_action,"last_cognition_reason":self.last_cognition_reason})
